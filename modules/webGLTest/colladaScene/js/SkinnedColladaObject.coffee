class window.SkinnedColladaObject extends ColladaObject
  app: null
  src: null
  scale: 1
  animations: null
  ...
  init: (scene) ->
    ...
    if @physics
      @obj = @skin = new Physijs.ConeMesh(aux.geometry, aux.material, @mass)
  ...
  update: (delta) =>
    if @skin && @currentAnimation
      ...
      #@hand.position.getPositionFromMatrix @skel.getObjectByName("ZOMBIE_R_Hand", true).matrix
